...
async function searchJavdb(id) {
    try {
        ...
        // 通过 env 环境变量传递 cookie，彻底避免 shell 特殊字符问题
        // 兼容 Windows 和 Linux
        const curlEnv = { ...process.env, JAVDB_COOKIE: cookie };
        ...
        -H "Cookie: $JAVDB_COOKIE"`,
            { timeout: 15000, maxBuffer: 1024 * 1024 * 10, env: curlEnv }
        ).toString();
